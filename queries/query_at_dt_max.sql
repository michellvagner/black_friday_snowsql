...
select
    tb_auth.tenant,
    tb_auth.port,
    max(tb_auth.date_time) date_time

from source_horizon.fdw.authorization tb_auth
left join
    (
    select
        import.fi_transaction_id,
        import.rule_name_strg,
        array_to_string(array_sort(array_remove(array_unique_agg(import.decision_xcd), '' ::VARIANT ), false ), ' and ') decision_xcd,
        import.tenant,
        import.test_xflg,
    ...
    group by import.tenant, import.rule_name_strg, import.fi_transaction_id, import.test_xflg
    qualify row_number() over (partition by fi_transaction_id order by max(import.created_dttm) asc) = 1

    ) as tb_adt

on tb_auth.fi_transaction_id = tb_adt.fi_transaction_id
...
and tb_auth.trn_typ <> '0'
and tb_auth.trn_amt > 0
