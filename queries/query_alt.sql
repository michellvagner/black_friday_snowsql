...
select
    ...
    tb_auth.tpec,
    demo_db.public.tpec_desc(tb_auth.tpec) tpec_desc,
    ...
    tb_adt.dcx r_dcn,
    demo_db.public.fx_score(ext_scor1, atni) fx_s,
    demo_db.public.fx_score(frd_scor, 'F') fx_fr_s,
    count(*) as amt,
...
and (
(
        date(date_time) between ( select inicio_mesmo_periodo_black_mes_passado_black_atual from source_horizon.fdw.tmp_parametrosdt ) and ( select fim_mesmo_periodo_black_mes_passado_black_atual from source_horizon.fdw.
)
or
(
...
GET @~/alt_fake_black_friday_ano_atual.parquet 'file://&caminho_saida/';